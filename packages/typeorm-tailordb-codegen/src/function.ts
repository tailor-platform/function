type RawColumnMetadata = {
  column: string;
  table: string;
  table_type: string; // 'r' table, 'v' view; mirrors Kysely codegen assumption
  schema: string;
  not_null: boolean;
  has_default: boolean;
  type: string; // Postgres-like type name
  type_schema: string;
...
function groupByTable(cols: RawColumnMetadata[]) {
  const map = new Map<string, { schema: string; name: string; isView: boolean; columns: RawColumnMetadata[] }>();
  for (const c of cols) {
...
function tsTypeFromPg(type: string): string {
  const t = type.toLowerCase();
  if (/(int|serial|smallint|bigint|decimal|numeric|double|real)/.test(t)) return 'number';
  if (/(bool|boolean)/.test(t)) return 'boolean';
  if (/(json|jsonb)/.test(t)) return 'any';
  if (/(date|time|timestamp)/.test(t)) return 'Date';
  if (/(uuid)/.test(t)) return 'string';
  if (/(char|text|varchar|citext)/.test(t)) return 'string';
  if (/(bytea)/.test(t)) return 'Buffer';
  return 'any';
}
...
function renderEntities(tables: ReturnType<typeof groupByTable>): string {
  const lines: string[] = [];

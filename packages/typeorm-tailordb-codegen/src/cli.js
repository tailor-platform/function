...
import "zx/globals";
program
    .name("typeorm-tailordb-codegen")
    .description("Generate TypeORM code for TailorDB")
...
console.log(`Generating code for TailorDB namespace "${namespace}" in app "${app}"...`);
const result = await $ `tailorctl workspace function test-run -a ${app} -g ${arg} -m ${machineuser} -s ${script}`
    .quiet()
    .nothrow();
if (!result.ok) {
    program.error(`${chalk.red.bold("ERROR")} tailorctl failed with exit code ${result.exitCode}.\n${result.stderr}`);
}
let json;
try {
    ...
    program.error(`${chalk.red.bold("ERROR")} Failed to parse output from tailorctl.`);
}
try {
